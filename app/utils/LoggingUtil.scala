...
trait LoggingUtil extends LoggerLike {
  ...
  def warnLogRes(message: => String)(implicit mc: MarkerContext, httpResponse: HttpResponse): Unit = this.warn(s"$message (${identifiersFromHttpResponse(httpResponse)})")
  ...
  def warnConnectorLog(message: => String)(implicit mc: MarkerContext, httpResponse: HttpResponse): Unit = this.warn(s"$message (${identifiersFromHttpResponse(httpResponse)})")

  def errorLog(message: => String)(implicit mc: MarkerContext, request: Request[_]): Unit = this.error(s"$message (${identifiers(request)})")

  def errorLogRes(message: => String)(implicit mc: MarkerContext, httpResponse: HttpResponse): Unit = this.error(s"$message (${identifiersFromHttpResponse(httpResponse)})")
